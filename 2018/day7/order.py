...
class Manager:
	...
	def add_to_sequence(self, step):
		self.sequence = self.sequence + step.get_label()
	...
	def remove_step(self, step):
		self.undone.remove((step.get_label(), step))
	...
	def build(self, workers):
		...
		while len(idle_workers) != len(workers) or self.get_undone() != []:
			...
			# print(time, end=' ')
			# for worker in workers:
			# 	if worker.get_step() != None:
			# 		print(worker.get_step().get_label(), end=' ')
			# 	else:
			# 		print('.', end=' ')
...
class Worker:
	...
	def execute_step(self, step):
		self.step = step
		# print('Working on step: ' + step.get_label())
		self.time_to_complete = 60 + ord(step.get_label()) - 63
	...
	def tick(self, instructions):
		if self.step != None:
			...
			if self.time_to_complete == 1:
				# print(self.step.get_label() + ' done')
...
def main():
	with open('input.txt', 'r') as raw_data:
		...
		print('Execute order: ' + manager.get_sequence())
