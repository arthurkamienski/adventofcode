...
(defn to-map [rule]
  (let [
    split (rest (re-find #"^(.*?) bags? contains? (.*?)\." rule))
    color (first split)
    contained (re-seq #"(\d+) ((?:\w\ ?)+) bag" (second split))]
  (letfn [(to-keyword [c]
