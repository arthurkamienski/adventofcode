...
type Input        = [MemValue]
type ASCIIInput   = [Char]
type Output       = [MemValue]
type ASCIIOutput  = [Char]
type Base         = MemValue
...
convertInput :: ASCIIInput -> Input
convertInput i = map convert i
  where
    convert c = toInteger $ fromEnum c

convertOutput :: Output -> ASCIIOutput
convertOutput o = map convert o
  where
    convert c = toEnum (fromInteger c) :: Char
...
executeASCII :: Program -> ASCIIInput -> Output
executeASCII p i = execute p (convertInput i)
...
startStateASCII :: Program -> ASCIIInput -> ProgramState
startStateASCII p i = ProgramState p 0 (convertInput i) [] 0 Start []
